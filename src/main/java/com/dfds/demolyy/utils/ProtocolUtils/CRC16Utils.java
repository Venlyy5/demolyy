...
public class CRC16Utils {
     ...
     * @return
     */
    public static String getCRC(String strHex16) {
        strHex16 = strHex16.trim().replace(" ", "");

        int len = strHex16.length();
        if (!(len % 2 == 0)) {
            return "0000";
        }
        int num = len / 2;
        ...
        for (int i = 0; i < num; i++) {
            int value = Integer.valueOf(strHex16.substring(i * 2, 2 * (i + 1)), 16);
            para[i] = (byte) value;
        }
        return getCRC(para);
    }
